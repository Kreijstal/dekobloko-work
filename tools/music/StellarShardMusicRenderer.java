...
public final class StellarShardMusicRenderer {
    private static final int SAMPLE_RATE = 22050;
    private static final int CHANNELS = 1;
    private static final int BUFFER_SAMPLES = 1024;
    private static final String[] TRACKS = {
        "music/Stellar_Shard_ingame",
        "music/Stellar_Shard_Halloween",
    };

    public static void main(String[] args) throws Exception {
        Path cache = Path.of(args.length > 0 ? args[0] : ".work/games/stellarshard/js5-index4-6-8-9/build10/stellarshard");
        Path outRoot = Path.of(args.length > 1 ? args[1] : ".work/games/stellarshard/music");
        Files.createDirectories(outRoot.resolve("wav"));

        ba.a(SAMPLE_RATE, false, 10);
        pf synth = archive(cache, 2);
        pf vorbis = archive(cache, 3);
        pf songs = archive(cache, 4);
        gk samples = new gk(synth, vorbis);

        for (String name : TRACKS) {
            jj sequence = ij.a(576, samples, name, songs, "");
            if (sequence == null) {
                throw new IllegalStateException("missing music track " + name);
            }

            d player = new d(sequence);
            disableLoop(player);
            player.d(64);
            player.c();
    ...
    private static pf archive(Path cache, int archive) {
        return new pf(new CacheBackend(cache, archive), true, 1);
    }

    private static void disableLoop(d player) throws ReflectiveOperationException {
        Field loop = d.class.getDeclaredField("ab");
        loop.setAccessible(true);
        loop.setBoolean(player, false);
    }

    private static byte[] render(hl mixer, d player) throws IOException {
    ...
    private static final class CacheBackend extends wf {
        ...
        lh a(int ignored) {
            ...
            try {
                ...
                index = new lh(raw, bg.a(raw.length, raw, 0), null);
                return index;
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
