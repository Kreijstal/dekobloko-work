...
| `confined` | 15 | One native/custom music load site found at `music/music`: `gi.a(new m(jd.b, v.Bb), ..., mn.b, "music/music", "")`, then playback through `new dd(te.c)`. Current cache/name lookup does not hydrate a valid `vk` sequence. |
...
| `stellarshard` | 11 | Two native/custom `jj` music loads found: `music/Stellar_Shard_ingame` and `music/Stellar_Shard_Halloween`, played through `new d(oe.a)`. Current cache/name lookup does not hydrate a valid `jj` sequence. |
...
| `terraphoenix` | 32 | Eight `tk` native sequence loads found: `terraphoenix_music_title`, `terraphoenix_music_title(no voices)`, `terraphoenix_music_ingame_rural`, `terraphoenix_music_ingame_urban`, `terraphoenix_music_ingame_alienbase`, `terraphoenix_jingle_success_high_scores`, `terraphoenix_jingle_fail_high_scores`, and `terraphoenix_jingle_mission_completed`. The client hydrates archive 5 songs against archives 3/4/6 through `uk.b.a(jj.c, new da(ee.G, mi.h), track, ...)`, so a dedicated `tk` renderer is still needed. |
| `torchallenge` | 12 | Eleven `ej` native-MIDI music loads found: `TOR_Lvl_1` through `TOR_Lvl_6`, `TOR_Boss_Fight`, `TOR_Title`, `TOR_Menu`, `TOR_lvl_start_jingle`, and `TOR_lvl_complete_jingle`. Boss `q`/`wg` names are sound effects. A dedicated `ej` renderer is still needed. |
...
| `voidhunters` | 26 | Four `kka` native sequence loads found: `VH_Title_Music`, `VH_Ingame_Music`, `VH_Jingle_Win`, and `VH_Jingle_Lose`. The client loads MIDI archive 21 and patch archive 20, then hydrates through `ew.p.k.a(..., pab.k[i])` before playback via `be.a(..., kka)`, so this still needs a dedicated `kka` renderer. |
