...
| `pool` | 20 | Native `cg -> vk` renderer scaffolded from the deobfuscated client. The current warmed cache exposes archives 8/9/11 but no archive 10 instrument index, and probed Pool builds 1-80 do not expose the `title`, `pool_modern`, table-theme, or jingle name hashes in JS5 metadata, so no Pool WAVs are verified yet. |
...
| `confined` | 15 | One native/custom music load site found at `music/music`: `gi.a(new m(jd.b, v.Bb), ..., mn.b, "music/music", "")`, then playback through `new dd(te.c)`. Build 15 exposes indexes 0-10/13, and probed Confined builds 1-80 do not expose the `music/music` name hash in JS5 metadata, so no WAV is verified. |
...
| `voidhunters` | 26 | Four `kka` native sequence loads found: `VH_Title_Music`, `VH_Ingame_Music`, `VH_Jingle_Win`, and `VH_Jingle_Lose`. The client expects MIDI archive 21 and patch archive 20, but build 26 exposes only indexes 0-8/11, and probed Void Hunters builds 1-80 do not expose these name hashes in JS5 metadata. |
