...
| `voidhunters` | 26 | Four `kka` native sequence loads found: `VH_Title_Music`, `VH_Ingame_Music`, `VH_Jingle_Win`, and `VH_Jingle_Lose`. The deobfuscated client expects sample archive 19, patch archive 20, and MIDI archive 21, but build 26 exposes only indexes 0-8/11. Probed Void Hunters builds through 700 do not expose the `VH_*` name hashes; build 68 has archive 21, but its payloads are JPEG resources rather than `kka` songs, while build 70 exposes archive 20 without a matching song archive. No renderer-generated soundtrack is available from the public JS5 payloads found so far. |
