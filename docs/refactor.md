...
| 2.4 accessor migration | **closed for dense; slab blocked only by a pre-existing failure** | The gate found three distinct defect classes, not one. **(a) Direct indexing**: `MethodHandle.js` (13 sites, it imported the accessors without using them) and `JitCompiler.getField`/`putField`, whose `*AtSite` siblings *in the same file* were already migrated -- so a per-file verdict is worthless, only a per-site gate finds these. Also the JRE classes that declare real guest fields (`Event`, `Dimension`, `Point`, `Insets`, `Rectangle`, `Button`, `Checkbox`, `Label`, `TextField`, `TextArea`, `_Guid`, `DDSurfaceDesc`, `DataInputStream`, `FilterInputStream`). **(b) Enumeration**, which the inventory missed entirely: `Object.keys(fields)` returns numeric indices under dense and omits every primitive field under slab (`objectModel.js`:218 says so and names `enumerateFieldKeys`). Real behaviour, not just diagnostics -- `String`/`StringBuilder` `fieldStringValue` and `HashMap`/`HashSet`/`TreeMap` key canonicalization -- plus env-gated field dumps in `callStack.js`, `JitCompiler.js` and `instructions/utils.js` that had silently stopped dumping what they exist to dump. **(c) The test fixtures themselves**: every remaining dense failure was a *test* writing `receiver.fields['Cls.f']` in setup or asserting on it directly, while production was already correct. Such a test writes and reads the same wrong way, so it stays green under the default layout and only reddens under the gate -- a red gate never implied a runtime defect, and I had been reading it as one. `src/jre` and `test/` are now at zero direct sites; the audit grep is three patterns, not one. **Result**: `JVM_DENSE_INSTANCE_FIELDS=1` full suite **10119 passing, 0 failures**, matching the default arm exactly. Under `JVM_WASM_HEAP=1 JVM_WASM_FIELDS=1`, six tier-preference assertions in `jitCompiler.test.js` still fail; confirmed **pre-existing** against a runnable HEAD (`e3adfdb`) -- the same six by name and number, 6 of 1973 there against 6 of 2364 here. The earlier "HEAD fails 8" is corrected to 6. Caveat on that baseline: HEAD could not run at all until `src/jre/index.js` was generated, the `.class` fixtures built, and the three untracked worker modules supplied. Without them a worktree crashes at `require` and reports **zero assertions**, which a failure-count check reads as a pass |
